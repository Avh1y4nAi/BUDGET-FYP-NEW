    ...
    return( //navbar JSX 
         <nav className = "navbar navbar-expand-lg navgar-custom sticky-top py-3"> 
            <div className = "container"> 
                <Link className = "navbar-brand navbar-brand-custom fs-3" to="/">Budgeter<span classname="text-accent">.</span></Link> {/* logo and anme with styling*/} 
                <button 
                        ...
                        {user ? ( // if user is not logged in 
                            ...
                            <>
                                <li className = "nav-item dropdown ms-lg-4 mt-lg-0"> 
                                    <button className="nav-link dropdown-toggle d-flex align-items-center gap-2 bg-light px-3 py-2 rounded-pill border-0"
                                    type="button"
                                    data-bs-toggle="dropdown">
                                    <img src = {User.profilePicture || 'https://via.placeholder.com/32' alt="profile" className="rounded-circle" style={{width:'32px', height:"32px", objectFit:"cover"}} /> {/*profile picture with styling and alternate*/}
                                </li>
                    </ul>
                </div>
            </div> 
